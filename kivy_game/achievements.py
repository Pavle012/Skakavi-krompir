...
class AchievementManager:
    def __init__(self):
        ...
        self.stats_file = os.path.join(dependencies.get_user_data_dir(), "stats.json")
    ...
    def update_stat(self, stat_name, value, incremental=True):
        ...
        new_unlocked = self.check_achievements()
        self.save_stats()
        return new_unlocked
...
def show_achievements_gui():
    ...
    for ach_id, defn in manager.definitions.items():
        ...
        rows.append((defn["title"], defn["description"], status))

    columns = [("Achievement", 0.30), ("Description", 0.45), ("Progress", 0.25)]

    extra_info = [f"Unlocked: {manager.get_unlocked_count()}/{manager.get_total_count()}"]
