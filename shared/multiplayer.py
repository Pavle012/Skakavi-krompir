...
class GameServer:
    ...
    def start(self, owner=None, host_name=None, description=None):
        self.directory_owner = owner
        session = friends_client.create_multiplayer_session(
            owner, host_name, description or "Skakavi Krompir host"
        )
        if not session:
            print("[Server] Failed to create multiplayer session")
            return False
        self.session_id = session["session_id"]
        self.running = True
        print(f"[Server] Multiplayer session created: {self.session_id}")
        return True
            
    def stop(self):
        self.running = False
        self.session_id = None
        if self.directory_owner:
    ...
    def register_with_directory(self, owner, host_name, description="Skakavi Krompir host"):
        if not owner or not host_name:
            return
        self.directory_registered = friends_client.register_host(
            owner, host_name, self.get_advertised_ip(), self.port, description
        )
        if not self.directory_registered:
            print("[Server] Host directory registration failed")

    def unregister_from_directory(self):
        if not self.directory_registered or not self.directory_owner:
            return
        if not friends_client.unregister_host(self.directory_owner):
            print("[Server] Host directory unregister failed")
        self.directory_registered = False
    ...
    def connect(self):
        try:
            self.socket = websocket.create_connection(
                friends_client.multiplayer_websocket_url(self.session_id), timeout=5
            )
    ...
    def listen_loop(self):
        ...
        while self.running:
            try:
                ...
                if not data:
                    break
                self.process_packet(json.loads(data))
            except Exception as e:
